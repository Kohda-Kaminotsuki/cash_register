...
def main():
    match sys.argv[1]:
        case "get":
            ...
            assert sys.argv[3] is None or (sys.argv[3].startswith("[") and sys.argv[3].endswith("]")), f'Invalid argument: third argument must be a list of floats or ints'
            if sys.argv[3] is not None: sys.argv[3] = [float(x) for x in (sys.argv[3].strip("[]").split(","))] # type: ignore
            if isinstance(sys.argv[3], list) and sys.argv[2] is "actual": print(sample_cash_register.get_actual_money(sys.argv[3]))
            if sys.argv[2] is "actual": print(sample_cash_register.get_actual_money())
            if isinstance(sys.argv[3], list) and sys.argv[2] is "percieved": print(sample_cash_register.get_percieved_money(sys.argv[3]))
            if sys.argv[2] is "percieved": print(sample_cash_register.get_percieved_money())
            sys.exit(0)
        ...
        case "remove": 
            ...
            if remaining_debt > 0:
                print(f"Removed {sys.argv[2]} from total, new total is {sample_cash_register.total}, but {remaining_debt} was left over")
            else:
